...
//		_Iterations("Iterations", Range(0,200)) = 100
//
//		// how far through space should we go before we stop
//		_ViewDistance("View Distance", Range(0,5)) = 2
//
//		// what colour should the background be
//		_SkyColor("Sky Color", Color) = (0.176, 0.487, 0.871, 1)
//
//		// what colour should the clouds be
//		_CloudColor("Cloud Color", Color) = (1, 1, 1, 1)
//
//		// density of clouds
//		_CloudDensity("Cloud Density", Range(0, 1)) = 0.5
...
//		// Source Note:
//		// "Note that the longer your view distance is, the more steps are required. And the smaller your clouds are, the bigger a render target is needed."
//		// "smaller the clouds" <- cloud density?
//
//
//	}
//
...
//			#include "UnityCG.cginc"
//
//			// global variables we're pulling in from Unity
//			float3    _CamPos;
//			float3    _CamRight;
//			float3    _CamUp;
//			float3    _CamForward;
//			float     _AspectRatio;
//			float     _FieldOfView;
//			sampler2D _NoiseOffsets; // noise texture
//
//			// local properties that get pulled in from the asset
//			int _Iterations;
//			float3 _SkyColor;
//			float4 _CloudColor;
//			float _ViewDistance;
...
//			};
//
//			// this method will be used for vertices
//			// appdata_base passes the position, normal, and one texture coordinate
...
//				v2f o;
//
//				// the vertex we want to multiply the vertex by Model View Projection matrix to get the on screen position
//				o.vertex = mul(UNITY_MATRIX_MVP, v.vertex);
//				
//				// the UV we take from the texcoord of appdata_base... what is that?
//				o.uv = v.texcoord;
//
//				// we return the created vert info
//				return o;
//			}
//
//			// Noise function by Inigo Quilez - https://www.shadertoy.com/view/4sfGzS
...
//			float fbm(float3 pos, int octaves) { float f = 0.; for (int i = 0; i < octaves; i++) { f += noise(pos) / pow(2, i + 1); pos *= 2.01; } f /= 1 - 1 / pow(2, octaves + 1); return f; }
//			
//			// this method is used to render the fragment
//			// v2f is what we defined above - it's the input
//			fixed4 frag(v2f i) : SV_Target
//			{
//				// half the uv?
//				float2 uv = (i.uv - 0.5) * _FieldOfView; //i.uv - 0.5;
//				
//				uv.x *= _AspectRatio;
//
//				// get a position a little far out from the camera
//				float3 pos = _CamPos * 0.4; // float3(0, 0, -5);
//
//				// ray is what - a projected ray into the scene
//				float3 ray = _CamUp * uv.y + _CamRight * uv.x + _CamForward;
//
//				float3 p = pos;
//
//				float density = 0;
//
//				// do thirty times - this is our marching
